...
namespace ImageMerger
{
    ...
    {
        private readonly HashSet<string> Extensions = new HashSet<string> {".png", ".jpg", ".jpeg"};
        ...
        private void ImageBut_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                if (dialog.ShowDialog() == System.Windows.Forms.DialogResult.OK) ImageDir.Text = dialog.SelectedPath;
            }
        }
        ...
        private void outButton_Click(object sender, RoutedEventArgs e)
        {
            using (var dialog = new FolderBrowserDialog())
            {
                if (dialog.ShowDialog() == System.Windows.Forms.DialogResult.OK) outDir.Text = dialog.SelectedPath;
            }
        }
        ...
        private async void DoneBT_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (ConcatMode.IsChecked.Value)
                bw.DoWork += (o, ev) => { ConcatImages(); };
            else
                bw.DoWork += (o, ev) => { SliceImages(); };
            bw.RunWorkerAsync();
        }
        ...
        private void ConcatImages()
        {
            ...
            var files = Directory.GetFiles(inDir).Where(x => Extensions.Contains(Path.GetExtension(x))).ToArray();
            ...
            while (files.Length != 0)
            {
                ...
                result.Save(path);
                result.Dispose();
                foreach (var file in mergingFiles) file.Dispose();
                i++;
            }
        }
        ...
        private void SliceImages()
        {
            ...
            if (!isTryTrueSlice)
                ...
                foreach (var imagePath in files)
                {
                    ...
                    {
                        var minMaxSlice = GetMinMaxHeight(previousSlice, image.Height, sliceCount, trueSliceMaxDistance,
                            sliceMinHeight, sliceMaxHeight, previousSlice);
        ...
        private (int Min, int Max) GetMinMaxHeight(int previousSlice, int imageHeight, int sliceCount,
            int trueSliceMaxDistance, int sliceMinHeight, int sliceMaxHeight, int absoluteMin)
        {
            if (sliceMaxHeight == 0)
            {
        ...
        private void Drop(object sender, DragEventArgs e)
        {
            ...
            if (tb != null) tb.Text = string.Format("{0}", ((string[]) text)[0]);
        }
        ...
        {
            if (TryTrueSlice.IsChecked.Value)
